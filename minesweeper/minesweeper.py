...
class Minesweeper():
    ...
    def is_mine(self, cell):
        """
        Uses the boolean characteristic of the board setup
        """
        i, j = cell
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.safes.add(cell)  # step 2

        # step 3
        # need to add nearby spaces to sentence
        sentence_cells = set()
        i = cell[0]
        j = cell[1]
        ...
        self.safes.update(safe_cells)

        # step 5
        for s1 in self.knowledge:
            for s2 in self.knowledge:
                if s2.cells.issubset(s1.cells) and s2 is not s1:
                    inferred_sentence = Sentence(
                        (s2.cells.difference(s1.cells)), s1.count-s2.count)
